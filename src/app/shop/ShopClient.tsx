  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row gap-12 transition-colors duration-500">
      ...
      <aside className="w-full md:w-64 flex-shrink-0">
        <h2 className="text-xl font-bold text-foreground mb-6 uppercase tracking-wider transition-colors duration-500">Categories</h2>
      ...
      <div className="flex-1">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-extrabold text-foreground transition-colors duration-500">Shop</h1>
          {region && (
            <div className="text-sm font-medium text-accent bg-accent/10 px-3 py-1 rounded-full border border-accent/20 transition-colors duration-500">
              Showing products for {region === 'US' ? 'United States' : 'Moldova'}
            </div>
          )}
        </div>
        ...
        {products.length === 0 ? (
          <div className="p-8 rounded-2xl border border-dashed border-border bg-surface flex flex-col items-center justify-center text-center transition-colors duration-500">
            ...
            <h3 className="text-lg font-bold text-foreground mb-2 transition-colors duration-500">No Products Found</h3>
            <p className="text-muted max-w-md transition-colors duration-500">The database hasn't been seeded yet. Please run the seed command to populate the store.</p>
          </div>
        ) : (
